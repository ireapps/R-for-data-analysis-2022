...
glimpse(poverty)
```
This gives us another view into our data, and tells us what data type each column is. Note the `$` character before each column name. This is a reminder that you can pull a column of data out of your table using the `$`: 
```{r}
poverty$population
